...
python experiments/06_advice_CoT/get_CoT.py \
    --prompt_file prompts/advice/CoT.txt \
    --test_dir data/added_labels/advice_test_examples \
    --output_file data/experiment_outputs/advice_CoT.csv
...
def get_user_prompt_from_df(df):
    """Since this is CoT, get 2 of the last client, therapist turns"""
    utterances = list(df.utterance_text)[-4:]
    interlocutors = list(df.interlocutor)[-4:]
    user_prompt = ''
