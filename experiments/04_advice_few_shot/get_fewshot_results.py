"""Script to run the few-shot prompt on advice test examples and get the 
results.

To run:

python experiments/04_advice_few_shot/get_fewshot_results.py \
    --prompt_file prompts/advice/few_shot.txt \
    --test_dir data/added_labels/advice_test_examples \
    --output_file data/experiment_outputs/advice_fewshot.csv

To run without fewshot(baseline):

python experiments/04_advice_few_shot/get_fewshot_results.py \
    --prompt_file prompts/advice/baseline.txt \
    --test_dir data/added_labels/advice_test_examples \
    --output_file data/experiment_outputs/advice_baseline.csv
...
def get_user_prompt_from_df(df):
    ...
    utterances = list(df.utterance_text)
    user_prompt = ''
    if len(utterances) > 1:
        user_prompt += f'Client: {utterances[-2]}'
    user_prompt += f'\nTherapist: {utterances[-1]}'
    return user_prompt
